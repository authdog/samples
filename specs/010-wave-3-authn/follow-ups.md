...
- [ ] **012** Wave 3 observability — signed webhook (`X-Authdog-Signature`) + Events API pull. Go/Python in-app. Java/C#: standalone HTTP receiver (no invented SDK helper). Secrets stay server-only.
- [ ] **013** Wave 3 lidar — same receiver rules; mark subject on a verified security event; `GET /sensitive` → 428 challenge, not a block. No Signals API.
- [ ] **014** Wave 4 authentication — `expo`, `ios-swift`, `android-kotlin`, `flutter-dart`. Identity only. Native stacks use the REST / redirect bridge until an official SDK ships. No `authz/<stack>`.
...
Start with **012**. MCP and agentic identity still need their own Spec Kit feature before any top-level folder.
