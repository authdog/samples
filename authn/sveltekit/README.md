# authdog authn on sveltekit

Smallest SvelteKit starter for Authdog identity using
`@authdog/sveltekit`. The handle hook reads the cookie; `getUser`
validates it on the profile page.
...
use, `http://localhost:5173/profile`). The server must set the
...
- **`locals.authdog.isAuthenticated` is not validation**: the handle
  hook only checks cookie presence. This sample uses `getUser` for the
  decision.
...
- **`getSession` is raw**: it returns the cookie value without
  userinfo validation. Don't use it to authorize.
- Do not log access tokens.
- Bearer tokens go only to a trusted HTTPS identity host.

## Next concept

Authorization for this stack is Planned: `authz/sveltekit`.
