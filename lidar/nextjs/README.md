# authdog lidar on nextjs

Smallest Next.js App Router starter for reacting to Lidar-relevant
...
Copy `.env.example` to `.env.local`. The variable has no
...
1. Create a webhook channel pointing at
   `https://<your-tunnel>/api/webhooks/authdog` and subscribe to sign-in
...
This completes Wave 1 for `nextjs`. See the catalog for other stacks.
