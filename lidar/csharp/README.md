# authdog lidar on csharp

Smallest .NET HTTP starter for reacting to Lidar-relevant identity
events with a step-up challenge. There is no official C# Lidar SDK —
this sample uses the documented HMAC surface. It does **not**
reference `Authdog.Sdk`.
...
This completes Wave 3 for `csharp`. See the catalog for other stacks.
