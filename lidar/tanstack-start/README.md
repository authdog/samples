# authdog lidar on tanstack-start

Smallest TanStack Start starter for reacting to Lidar-relevant
identity events with a step-up challenge.
...
2. `curl -H "X-Demo-User: u123" http://localhost:3000/api/sensitive` →
   `428 Step-up required` with a challenge payload.
3. `curl -X POST -H "X-Demo-User: u123" http://localhost:3000/api/step-up/complete`
   → clears the mark.
...
This completes Wave 2 for `tanstack-start`. See the catalog for other
stacks.
