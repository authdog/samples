# authdog lidar on astro

Smallest Astro SSR starter for reacting to Lidar-relevant identity
...
no `PUBLIC_` prefix. Copy `.env.example` to `.env`.
...
1. Create a webhook channel pointing at
   `https://<your-tunnel>/api/webhooks/authdog` and subscribe to
   sign-in events. Send a test event whose `data.user.id` is `u123`.
2. `curl -H "X-Demo-User: u123" http://localhost:4321/api/sensitive` →
   `428 Step-up required` with a challenge payload.
3. `curl -X POST -H "X-Demo-User: u123" http://localhost:4321/api/step-up/complete`
   → clears the mark.
...
- **Raw body first**: the endpoint reads `await request.text()` so the
  HMAC is computed over the exact bytes. Parsing JSON first breaks
  verification.
- **SSR required**: API routes need `output: "server"` or `"hybrid"`.
...
This completes Wave 2 for `astro`. See the catalog for other stacks.
