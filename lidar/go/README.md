...
Smallest Gin starter for reacting to Lidar-relevant identity events
with a step-up challenge. There is no official Go Lidar SDK — this
sample uses the documented HMAC surface.
...
- **Raw body first**: `GetRawData` keeps the exact bytes.
...
This completes Wave 3 for `go`. See the catalog for other stacks.
