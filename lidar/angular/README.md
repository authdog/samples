# authdog lidar on angular

Smallest Lidar companion for an Angular SPA: a standalone Node server
...
Signals API. The signing secret must never reach the Angular bundle,
so this sample is a small Node server, not Angular code. It implements
the documented recipe: react with a challenge, not a block.
...
The companion listens on http://localhost:3001 (the Angular dev server
stays on 4200).
...
This completes Wave 2 for `angular`. See the catalog for other stacks.
