# authdog lidar on sveltekit

Smallest SvelteKit starter for reacting to Lidar-relevant identity
...
2. `curl -H "X-Demo-User: u123" http://localhost:5173/sensitive` →
   `428 Step-up required` with a challenge payload.
3. `curl -X POST -H "X-Demo-User: u123" http://localhost:5173/step-up/complete`
   → clears the mark.
...
This completes Wave 2 for `sveltekit`. See the catalog for other
stacks.
