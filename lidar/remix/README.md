# authdog lidar on remix

Smallest Remix starter for reacting to Lidar-relevant identity events
...
- **Raw body first**: the action reads `await request.text()` so the
...
This completes Wave 1 for `remix`. See the catalog for other stacks.
