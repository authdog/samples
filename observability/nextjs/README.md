# authdog observability on nextjs

Smallest Next.js App Router starter for Authdog observability: a
signed-webhook route handler and an Events API read path.
...
All four are server-only secrets. Copy `.env.example` to `.env.local`.
These variables have no `NEXT_PUBLIC_` prefix, so they never reach the
browser bundle.
...
1. Create a webhook channel pointing at
   `https://<your-tunnel>/api/webhooks/authdog` (use a tunnel for local
...
Lidar integration for this stack is Planned: `lidar/nextjs`.
