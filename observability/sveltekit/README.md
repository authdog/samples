# authdog observability on sveltekit

Smallest SvelteKit starter for Authdog observability: a signed-webhook
endpoint and an Events API read path.
...
All four are server-only secrets — no `PUBLIC_` prefix, so SvelteKit
keeps them out of the client bundle. Copy `.env.example` to `.env`.
...
1. Create a webhook channel pointing at
   ...
   The server logs the verified event type and delivery ID.
2. `curl http://localhost:5173/events` → the first page of events and
...
Lidar integration for this stack is Planned: `lidar/sveltekit`.
