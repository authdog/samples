# authdog observability on node

Smallest plain-Node observability starter: a standalone signed-webhook
receiver and an Events API poller, with no framework.
...
All four are server-only secrets. Set them in your shell or a `.env`
you load yourself.
...
```bash
npm install
npm run receive   # webhook receiver on :3000
npm run events    # one Events API page
```
...
2. `npm run events` → the first page of events and the opaque `after`
   cursor. `npm run events -- <cursor>` → the next page.
...
Lidar integration for this stack is Planned: `lidar/node`.
