# authdog observability on astro

Smallest Astro SSR starter for Authdog observability: a signed-webhook
...
All four are server-only secrets — no `PUBLIC_` prefix, so Astro keeps
them out of the client bundle. Copy `.env.example` to `.env`.
...
2. `curl http://localhost:4321/api/events` → the first page of events
...
Lidar integration for this stack is Planned: `lidar/astro`.
