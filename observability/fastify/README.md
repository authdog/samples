# authdog observability on fastify

Smallest Fastify starter for Authdog observability: a signed-webhook
route and an Events API read path.
...
- **Raw body first**: a custom content parser keeps
  `application/json` as a buffer so the HMAC is computed over the
  exact bytes. Parsing JSON first breaks verification.
...
Lidar integration for this stack is Planned: `lidar/fastify`.
