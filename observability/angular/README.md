# authdog observability on angular

Smallest observability companion for an Angular SPA: a standalone
...
**server-side** — the signing secret must never reach the Angular
bundle, so this sample is a small Node receiver, not Angular code.
Your SPA stays unaware of it; the receiver is where you react to
events (audit, alerting, sync).
...
The receiver listens on http://localhost:3001 (the Angular dev server
stays on 4200).
...
- **Server-side only**: never put the signing secret in the Angular
  app or `environment.ts`. This receiver is a separate Node process.
...
Lidar integration for this stack is Planned: `lidar/angular`.
